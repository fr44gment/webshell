...
export class Log extends Factory {
    ...
    constructor(id, configuration) {
        ...
        this._outputGuard = (verbosity) => (Log.VERBOSITY_LOOKUP.indexOf(this._configuration.verbosity)
         ...
         */
        this._getEscapeCode = (escapeCodeFormatSymbol) => (this._configuration.formattingFlag)
         ...
         * @returns {string} An empty string.
         */
        this._createAsciiString = (asciiSymbol) => (this._configuration.asciiFlag)
         ...
         * @returns {string} An empty string.
         */
        this._createDateTimeString = () => (this._configuration.dateTimeFlag)
         ...
         * @returns {string} An empty string.
         */
        this._createInstanceIdString = () => (this._configuration.instanceIdFlag)
            ? `${this._createAsciiString('WRAP_LEFT')}`
                + `${this._getEscapeCode('FG_GREY')}`
                + `#${this._instanceId}`
                + `${this._getEscapeCode('RESET')}`
         ...
         * @returns {string} An empty string.
         */
        this._createPrimaryTextString = () => (this._configuration.primaryTextFlag)
            ? `${this._createAsciiString('WRAP_LEFT')}`
         ...
         * @returns {string} An empty string.
         */
        this._createSecondaryTextString = () => (this._configuration.secondaryTextFlag)
            ? `${this._createAsciiString('WRAP_LEFT')}`
         ...
         */
        this._createMessageString = (message, formatCode) => `${formatCode}${message}${this._getEscapeCode('RESET')}`;
        /**
         ...
         */
        this._createLogString = (message, messageFormatCode, context) => `${this._createDateTimeString()}` // Date
    ...
    trace(message, context) {
        if (this._outputGuard('TRACE'))
            Log._processMessage(message, line => process.stdout.write(this._createLogString(line, this._getEscapeCode('FG_GREY'), context)));
        return true;
    }
    /**
     * @method Log#info
    ...
    info(message, context) {
        if (this._outputGuard('INFO'))
            Log._processMessage(message, line => process.stdout.write(this._createLogString(line, this._getEscapeCode('FG_WHITE'), context)));
        return true;
    }
    /**
     * @method Log#warn
    ...
    warn(message, context) {
        if (this._outputGuard('WARN'))
            Log._processMessage(message, (line) => process.stdout.write(this._createLogString(line, this._getEscapeCode('FG_ORANGE'), context)));
        return true;
    }
    /**
     * @method Log#error
    ...
    error(message, context) {
        if (this._outputGuard('ERROR'))
            Log._processMessage(message, (line) => process.stdout.write(this._createLogString(line, this._getEscapeCode('FG_RED'), context)));
        return false;
    }
    /**
     * @method Log#fatal
    ...
    fatal(message, context) {
        if (this._outputGuard('FATAL'))
            Log._processMessage(message, (line) => process.stdout.write(this._createLogString(line, `${this._getEscapeCode('BG_RED')}${this._getEscapeCode('FG_BLACK')}`, context)));
        process.exit(1);
    }
    /**
    ...
    httpRequest(request) {
        return this.trace(`(${request.method}) `
            ...
            + `${request.protocol}://${request.get('host')}${request.originalUrl}`, 'http');
    }
    /**
     * @method Log#database
    ...
    database(url, dataId, method) {
        return this.trace(`${method} ${dataId} -> ${url}`, 'database');
    }
    /**
     * @method Log#email
    ...
    email(emailId, recipient) {
        return this.trace(`${emailId} -> ${recipient}`, 'email');
    }
}
 ...
 */
Log.VERBOSITY_LOOKUP = ['TRACE', 'INFO', 'WARN', 'ERROR', 'FATAL'];
/**
 * @memberof Log
...
Log._processMessage = (message, callback) => message.split('\n').forEach(callback);
/**
 * Change history:
